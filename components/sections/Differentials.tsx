...
export function Differentials() {
  const [highlightedIndex, setHighlightedIndex] = useState<number>(2); // Starts on card 3 ("Sob Medida")
    ...
    >
      <div className="container-site">
        ...
        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[var(--space-6)]">
          {items.map((item, i) => {
            const active = hoveredIndex !== null ? hoveredIndex === i : highlightedIndex === i;
            return (
              <StaggerItem key={item.num}>
                <div
                  onMouseEnter={() => setHoveredIndex(i)}
                  onMouseLeave={() => setHoveredIndex(null)}
                  className={`group p-[var(--space-6)] rounded-[var(--radius-sm)] border h-full flex flex-col gap-4 transition-all duration-500 cursor-default ${
                    active
                      ? "bg-[var(--color-gold)] border-[var(--color-gold)] text-[var(--color-ink)] scale-[1.02] shadow-[0_10px_30px_rgba(232,196,76,0.15)]"
                      : "bg-[var(--color-bg-soft)] border-[var(--color-line)] text-[var(--color-text)]"
                  }`}
                >
                  <span
                  ...
                  <h3
                    className={`font-['Clash_Display',sans-serif] text-[clamp(1.25rem,2vw,1.625rem)] font-semibold transition-colors duration-300 ${
                      active ? "text-[var(--color-ink)]" : "text-[var(--color-text)]"
                    }`}
                  >
                    {item.title}
                  </h3>
